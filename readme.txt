...
* **Every send is now recorded** in `ansp_notify_log` (project, title, recipients, count, who triggered it, when; capped at 50). On 2026-09-04 there was no way to answer "who was emailed, and how many times" - the sending left no trace at all.
* **Fixed: `portal/settings` addressed three calendar options that no longer exist and hid the two that do.** It still listed `ansp_calendar_main`, `_small` and `_friday`; since 1.32.0 a calendar option is named after its group (`ansp_calendar_ans`, `ansp_calendar_cs`). The key list is now derived from the live group tree.
